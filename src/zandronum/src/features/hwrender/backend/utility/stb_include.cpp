...
#include <cstring>
#include <cstdint>
#include <utility> // [rc4l] std::move; upstream got it transitively.

static bool stb_include_load_file(FString filename, FString &out)
{
	// [rc4l] Their FindFile is a full-path lookup; CheckNumForFullName is our equivalent.
	int f = fileSystem.CheckNumForFullName(filename.GetChars());
	if(f < 0) return false;
