...
#include "zx_texconst.h" // [rc4l] TM_* texture modes.
#include "hw_material.h" // [rc4l] DefaultRenderStyle().
#include "i_system.h"
#include "intrect.h"
#include "zx_video.h" // [rc4l] zx_screen, for the 2D vertex buffer.
#include <algorithm>
// [rc4l] Their tree has these in scope from its own headers; ours does not.
using std::min;
using std::max;
namespace pi { constexpr double pi() { return 3.14159265358979323846; } }
// [rc4l] Their VM abort path; without the ZScript VM these become fatal errors.
enum { X_OTHER = 0, X_ARRAY_OUT_OF_BOUNDS = 1 };
#define ThrowAbortException(code, ...) I_Error(__VA_ARGS__)
// [rc4l] VM null-check macro; without the VM a plain guard is enough.
#define PARAM_NULLCHECK(ptr, name) ((ptr) != nullptr ? (ptr) : (I_Error("null %s", #name), (ptr)))
#include "vectors.h"
// [rc4l] vm.h dropped with the script exports; we do not port the ZScript VM.
#include "c_cvars.h"
#include "v_draw.h"
#include "v_video.h"
#include "fcolormap.h"
...
// [rc4l] FCanvas is a script-facing DObject; not registered without the ZScript VM.

// [rc4l] DShape2DTransform is a script-facing DObject; not registered without the ZScript VM.
...
// [rc4l] DShape2D is a script-facing DObject; not registered without the ZScript VM.
...
{
	// [rc4l] Their ActiveRatio corrects for non-square pixels; ours are square, so this is the
	// plain display aspect.
	float ar = 4.f / 3.f / ((float)zx_screen->GetWidth() / (float)zx_screen->GetHeight());
	float sw = 320.f * classic_scaling_factor / (float)zx_screen->GetWidth() / ar;
	return sw;
}

float F2DDrawer::GetClassicFlatScalarHeight()
{
	float sh = 240.f / classic_scaling_pixelaspect * classic_scaling_factor / (float)zx_screen->GetHeight();
	return sh;
}
...
{
	mVertexBuffer = zx_screen->CreateVertexBuffer();
	mIndexBuffer = zx_screen->CreateIndexBuffer();
...
//==========================================================================
//
// [rc4l] The script-facing texture-canvas API (AllCanvases, InitTextureCanvasGC,
// GetTextureCanvas) is dropped. It exists so ZScript can draw into a canvas texture, needs the VM's
// GC to mark the canvases, and pulls in their FGameTexture/FTextureManager split. Nothing in our
// engine calls it.
//
//==========================================================================
