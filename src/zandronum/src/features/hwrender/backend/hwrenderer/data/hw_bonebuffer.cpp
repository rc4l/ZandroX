...
BoneBuffer::BoneBuffer(int pipelineNbr) : mPipelineNbr(pipelineNbr)
{
	...
	{
		mBufferType = false;
		mBlockSize = zx_screen->maxuniformblock / BONE_SIZE;
		mBlockAlign = zx_screen->uniformblockalignment < BONE_SIZE ? 1 : zx_screen->uniformblockalignment / BONE_SIZE;
		mMaxUploadSize = (mBlockSize - mBlockAlign);
	}

	for (int n = 0; n < mPipelineNbr; n++)
	{
		mBufferPipeline[n] = zx_screen->CreateDataBuffer(BONEBUF_BINDINGPOINT, mBufferType, false);
