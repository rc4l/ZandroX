...
void VRMode::AdjustViewport(ZXFrameBuffer *screen) const
{
	zx_screen->mSceneViewport.height = (int)(zx_screen->mSceneViewport.height * mVerticalViewportScale);
	zx_screen->mSceneViewport.top = (int)(zx_screen->mSceneViewport.top * mVerticalViewportScale);
	zx_screen->mSceneViewport.width = (int)(zx_screen->mSceneViewport.width * mHorizontalViewportScale);
	zx_screen->mSceneViewport.left = (int)(zx_screen->mSceneViewport.left * mHorizontalViewportScale);

	zx_screen->mScreenViewport.height = (int)(zx_screen->mScreenViewport.height * mVerticalViewportScale);
	zx_screen->mScreenViewport.top = (int)(zx_screen->mScreenViewport.top * mVerticalViewportScale);
	zx_screen->mScreenViewport.width = (int)(zx_screen->mScreenViewport.width * mHorizontalViewportScale);
	zx_screen->mScreenViewport.left = (int)(zx_screen->mScreenViewport.left * mHorizontalViewportScale);
}

VSMatrix VRMode::GetHUDSpriteProjection() const
{
	VSMatrix mat;
	int w = zx_screen->GetWidth();
	int h = zx_screen->GetHeight();
...
VSMatrix VREyeInfo::GetProjection(float fov, float aspectRatio, float fovRatio, bool iso_ortho) const
{
	...
	{
		double zNear = -3.0/fovRatio; // zx_screen->GetZNear();
		double zFar = zx_screen->GetZFar();
	...
	else if (mShiftFactor == 0)
	{
		...
		result.perspective(fovy, aspectRatio, zx_screen->GetZNear(), zx_screen->GetZFar());
		return result;
	}
	else
	{
		double zNear = zx_screen->GetZNear();
		double zFar = zx_screen->GetZFar();
