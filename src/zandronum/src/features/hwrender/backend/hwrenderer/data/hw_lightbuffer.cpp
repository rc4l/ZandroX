	...
	mPipelineNbr(pipelineNbr)
{
	...
	{
		mBufferType = false;
		mBlockSize = zx_screen->maxuniformblock / ELEMENT_SIZE;
		mBlockAlign = zx_screen->uniformblockalignment < ELEMENT_SIZE ? 1 : zx_screen->uniformblockalignment / ELEMENT_SIZE;
		mMaxUploadSize = (mBlockSize - mBlockAlign);

		//mByteSize += zx_screen->maxuniformblock;	// to avoid mapping beyond the end of the buffer. REMOVED this...This can try to allocate 100's of MB..
	}

	for (int n = 0; n < mPipelineNbr; n++)
	{
		mBufferPipeline[n] = zx_screen->CreateDataBuffer(LIGHTBUF_BINDINGPOINT, mBufferType, false);
