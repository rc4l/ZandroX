...
void IShadowMap::UploadLights()
{
	...
	if (mLightList == nullptr)
		mLightList = zx_screen->CreateDataBuffer(LIGHTLIST_BINDINGPOINT, true, false);
...
void IShadowMap::UploadAABBTree()
{
	if (mNewTree)
	{
		...
		if (!mNodesBuffer)
			mNodesBuffer = zx_screen->CreateDataBuffer(LIGHTNODES_BINDINGPOINT, true, false);
		mNodesBuffer->SetData(mAABBTree->NodesSize(), mAABBTree->Nodes(), BufferUsageType::Static);

		if (!mLinesBuffer)
			mLinesBuffer = zx_screen->CreateDataBuffer(LIGHTLINES_BINDINGPOINT, true, false);
