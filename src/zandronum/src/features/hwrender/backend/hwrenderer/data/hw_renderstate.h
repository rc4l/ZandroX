...
public:
	// [rc4l] Takes our FMaterial directly; upstream's FGameTexture overloads (setSeen/PreBindTexture/upscaling) are dropped since Zandronum's scene layer already resolves its own materials.
	void SetMaterial(FMaterial *mat, int clampmode, int translation, int overrideshader)
	{
		...
		mTextureModeFlags = ZX_MaterialLayerFlags(mat);
		auto scale = ZX_MaterialDetailScale(mat);
		mStreamData.uDetailParms = { scale.X, scale.Y, 2, 0 };
	}

	// [rc4l] UZDoom's texture-taking overload, which their 2D emitter calls. Their upscale and
	// PreBindTexture machinery belongs to the texture system we have not ported (issue #4), so this
	// validates the material and defers to the overload above.
	void SetMaterial(FGameTexture* tex, EUpscaleFlags upscalemask, int scaleflags, int clampmode, int translation, int overrideshader)
	{
		FMaterial *mat = FMaterial::ValidateTexture(tex);
		if (mat != nullptr) SetMaterial(mat, clampmode, translation, overrideshader);
	}
