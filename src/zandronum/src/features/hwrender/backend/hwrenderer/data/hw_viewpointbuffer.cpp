	...
	mPipelineNbr(pipelineNbr)
{
	...
	mBlockAlign = ((sizeof(HWViewpointUniforms) / zx_screen->uniformblockalignment) + 1) * zx_screen->uniformblockalignment;
	mByteSize = mBufferSize * mBlockAlign;

	for (int n = 0; n < mPipelineNbr; n++)
	{
		mBufferPipeline[n] = zx_screen->CreateDataBuffer(VIEWPOINT_BINDINGPOINT, false, true);
