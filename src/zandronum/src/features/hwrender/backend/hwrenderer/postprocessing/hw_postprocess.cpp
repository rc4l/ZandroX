...
void PPBloom::RenderBloom(PPRenderState *renderstate, int sceneWidth, int sceneHeight, int fixedcm)
{
	...
	extractUniforms.Scale = zx_screen->SceneScale();
	extractUniforms.Offset = zx_screen->SceneOffset();
	...
	renderstate->Shader = &BloomCombine;
	renderstate->Uniforms.Clear();
	renderstate->Viewport = zx_screen->mSceneViewport;
...
void PPBloom::RenderBlur(PPRenderState *renderstate, int sceneWidth, int sceneHeight, float gameinfobluramount)
{
	...
	renderstate->Shader = &BloomCombine;
	renderstate->Uniforms.Clear();
	renderstate->Viewport = zx_screen->mScreenViewport;
	renderstate->SetInputTexture(0, &level0.VTexture, PPFilterMode::Linear);
	renderstate->SetOutputCurrent();
	renderstate->SetNoBlend();
	renderstate->Draw();
...
void PPLensDistort::Render(PPRenderState *renderstate)
{
	...
	float aspect = zx_screen->mSceneViewport.width / (float)zx_screen->mSceneViewport.height;
...
void PPFXAA::Render(PPRenderState *renderstate)
{
	...
	uniforms.ReciprocalResolution = { 1.0f / zx_screen->mScreenViewport.width, 1.0f / zx_screen->mScreenViewport.height };
...
{
	return zx_screen->glslversion >= 4.f ? 400 : 330;
}
...
void PPAmbientOcclusion::Render(PPRenderState *renderstate, float m5, int sceneWidth, int sceneHeight)
{
	...
	auto sceneScale = zx_screen->SceneScale();
	auto sceneOffset = zx_screen->SceneOffset();
	...
	linearUniforms.LinearizeDepthA = 1.0f / zx_screen->GetZFar() - 1.0f / zx_screen->GetZNear();
	linearUniforms.LinearizeDepthB = max(1.0f / zx_screen->GetZNear(), 1.e-8f);
	...
	combineUniforms.Scale = zx_screen->SceneScale();
	combineUniforms.Offset = zx_screen->SceneOffset();
...
void PPShadowMap::Update(PPRenderState* renderstate)
{
	...
	uniforms.NodesCount = zx_screen->mShadowMap.NodesCount();
