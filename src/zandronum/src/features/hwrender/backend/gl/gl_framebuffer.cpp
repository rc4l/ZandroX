...
void OpenGLFrameBuffer::InitializeState()
{
	...
	mVertexData = new FFlatVertexBuffer(GetWidth(), GetHeight(), zx_screen->mPipelineNbr);
	mSkyData = new FSkyVertexBuffer;
	mViewpoints = new HWViewpointBuffer(zx_screen->mPipelineNbr);
	mLights = new FLightBuffer(zx_screen->mPipelineNbr);
	mBones = new BoneBuffer(zx_screen->mPipelineNbr);
...
void OpenGLFrameBuffer::AmbientOccludeScene(float m5)
{
	...
	glViewport(zx_screen->mSceneViewport.left, mSceneViewport.top, mSceneViewport.width, mSceneViewport.height);
...
void OpenGLFrameBuffer::UpdateShadowMap()
{
	if (mShadowMap.PerformUpdate())
	{
		...
		static_cast<GLDataBuffer*>(zx_screen->mShadowMap.mLightList)->BindBase();
		static_cast<GLDataBuffer*>(zx_screen->mShadowMap.mNodesBuffer)->BindBase();
		static_cast<GLDataBuffer*>(zx_screen->mShadowMap.mLinesBuffer)->BindBase();

		GLRenderer->mBuffers->BindShadowMapFB();

		GLRenderer->mShadowMapShader->Bind();
		GLRenderer->mShadowMapShader->Uniforms->ShadowmapQuality = gl_shadowmap_quality;
		GLRenderer->mShadowMapShader->Uniforms->NodesCount = zx_screen->mShadowMap.NodesCount();
		...
		const auto& viewport = zx_screen->mScreenViewport;
		glViewport(viewport.left, viewport.top, viewport.width, viewport.height);

		GLRenderer->mBuffers->BindShadowMapTexture(16);
		FGLDebug::PopGroup();
		zx_screen->mShadowMap.FinishUpdate();
	}
}
