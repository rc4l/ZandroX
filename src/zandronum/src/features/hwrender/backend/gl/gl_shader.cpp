...
// [rc4l] The on-disk program-binary cache needs UZDoom's FileReader::OpenFile/ReadUInt32 and FileWriter, which our tree does not have; gated off until that file API is ported.
#define ZX_SHADER_BINARY_CACHE 0
...
//
	...
	assert(zx_screen->mLights != NULL);
	assert(zx_screen->mBones != NULL);


	if ((gl.flags & RFL_SHADER_STORAGE_BUFFER) && zx_screen->allowSSBO())
		vp_comb << "#version 430 core\n#define SUPPORTS_SHADOWMAPS\n";
	else
		vp_comb << "#version 330 core\n";

	bool lightbuffertype = zx_screen->mLights->GetBufferType();
	if (!lightbuffertype)
		vp_comb.AppendFormat("#define NUM_UBO_LIGHTS %d\n#define NUM_UBO_BONES %d\n", zx_screen->mLights->GetBlockSize(), zx_screen->mBones->GetBlockSize());
	else
	...
	if (proc_prog_lump != NULL)
	{
		...
		if (*proc_prog_lump != '#')
		{
			...
			if (pp_data.IndexOf("ProcessMaterial") < 0 && pp_data.IndexOf("SetupMaterial") < 0)
			{
				...
				{
					int pl_lump = fileSystem.CheckNumForFullName("shaders/hwrender/func_defaultmat2.fp", 0);
					if (pl_lump == -1) I_Error("Unable to load '%s'", "shaders/hwrender/func_defaultmat2.fp");
					fp_comb << "\n" << GetStringFromLump(pl_lump);
				}
				else
				{
					int pl_lump = fileSystem.CheckNumForFullName("shaders/hwrender/func_defaultmat.fp", 0);
					if (pl_lump == -1) I_Error("Unable to load '%s'", "shaders/hwrender/func_defaultmat.fp");
					fp_comb << "\n" << GetStringFromLump(pl_lump);
			...
			if (pp_data.IndexOf("ProcessLight") < 0)
			{
				int pl_lump = fileSystem.CheckNumForFullName("shaders/hwrender/func_defaultlight.fp", 0);
				if (pl_lump == -1) I_Error("Unable to load '%s'", "shaders/hwrender/func_defaultlight.fp");
				fp_comb << "\n" << GetStringFromLump(pl_lump);
			}
...
FShader *FShaderCollection::Compile (const char *ShaderName, const char *ShaderPath, const char *LightModePath, const char *shaderdefines, bool usediscard, EPassType passType)
{
	...
	try
	{
		...
		if (!shader->Load(ShaderName, "shaders/hwrender/main.vp", "shaders/hwrender/main.fp", ShaderPath, LightModePath, defines.GetChars()))
		{
