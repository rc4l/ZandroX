...
{
	auto buffer = static_cast<GLVertexBuffer *>(zx_screen->mVertexData->GetBufferObjects().first);
	buffer->Bind(nullptr);
...
void FGLRenderer::PostProcessScene(int fixedcm, float flash, const std::function<void()> &afterBloomDrawEndScene2D)
{
	...
	renderstate.Time = static_cast<float>(zx_screen->FrameTime / 1000.0);
...
{
	zx_screen->Draw2D();	// draw all pending 2D stuff before copying the buffer
	twod->Clear();
	...
	{
		ClearBorders();
		box = zx_screen->mOutputLetterbox;
	}
...
void FGLRenderer::DrawPresentTexture(const IntRect &box, bool applyGamma)
{
	...
	mPresentShader->Uniforms->Scale = { zx_screen->mScreenViewport.width / (float)mBuffers->GetWidth(), zx_screen->mScreenViewport.height / (float)mBuffers->GetHeight() };
...
{
	const auto &box = zx_screen->mOutputLetterbox;
