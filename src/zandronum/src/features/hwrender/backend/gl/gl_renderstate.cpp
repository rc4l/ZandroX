...
bool FGLRenderState::ApplyShader()
{
	...
	activeShader->muTimer.Set((double)(zx_screen->FrameTime - firstFrame) * (double)mShaderTimer / 1000.);
	...
	if (!zx_screen->mLights->GetBufferType() && index >= 0)
	{
		size_t start, size;
		index = zx_screen->mLights->GetBinding(index, &start, &size);

		if (start != mLastMappedLightIndex || zx_screen->mPipelineNbr > 1) // If multiple buffers always bind
		{
			mLastMappedLightIndex = start;
			static_cast<GLDataBuffer*>(zx_screen->mLights->GetBuffer())->BindRange(nullptr, start, size);
		}
	}
	...
	if (!zx_screen->mBones->GetBufferType() && index >= 0) // Uniform buffer fallback support
	{
		size_t start, size;
		index = zx_screen->mBones->GetBinding(index, &start, &size);

		if (start != mLastMappedBoneIndexBase || zx_screen->mPipelineNbr > 1) // If multiple buffers always bind
		{
			mLastMappedBoneIndexBase = start;
			static_cast<GLDataBuffer*>(zx_screen->mBones->GetBuffer())->BindRange(nullptr, start, size);
		}
	}
...
void FGLRenderState::ApplyMaterial(FMaterial *mat, int clampmode, int translation, int overrideshader)
{
	...
	// [rc4l] Upstream assumes layer 0 always exists. Ours comes from the FTexture->FMaterial adapter
	// and can legitimately fail (a texture our path has not uploaded yet), so skip rather than
	// dereference null -- which the crash handler turns into a spin, not a clean fault.
	if (base == nullptr || layer == nullptr) return;
...
void FGLRenderState::SetStencil(int offs, int op, int flags = -1)
{
	...
	glStencilFunc(GL_EQUAL, zx_screen->stencilValue + offs, ~0);		// draw sky into stencil
...
void FGLRenderState::Clear(int targets)
{
	...
	if (targets & CT_Color)
	{
		...
		glClearColor(zx_screen->mSceneClearColor[0], zx_screen->mSceneClearColor[1], zx_screen->mSceneClearColor[2], zx_screen->mSceneClearColor[3]);
	}
	glClear(gltarget);
}
...
void FGLRenderState::ClearScreen()
{
	...
	zx_screen->mViewpoints->Set2D(*this, SCREENWIDTH, SCREENHEIGHT);
	SetColor(0, 0, 0);
	Apply();
