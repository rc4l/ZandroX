...
void GLPPRenderState::Draw()
{
	...
	{
		if (!shader->Uniforms)
			shader->Uniforms.reset(zx_screen->CreateDataBuffer(POSTPROCESS_BINDINGPOINT, false, false));
	...
	glViewport(zx_screen->mScreenViewport.left, zx_screen->mScreenViewport.top, zx_screen->mScreenViewport.width, zx_screen->mScreenViewport.height);
}
