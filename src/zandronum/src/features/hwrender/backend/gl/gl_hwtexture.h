...
public:
	...
	// [rc4l] Adopt a GL texture already uploaded by Zandronum's own texture path, so Bind() short-circuits and BindOrCreate() never needs our pixel buffers (see issue #4).
	void AdoptTextureHandle(unsigned int texid) { glTexID = texid; }
