...
void FGLRenderer::PresentAnaglyph(bool r, bool g, bool b)
{
	...
	mBuffers->BindEyeTexture(0, 0);
 	DrawPresentTexture(zx_screen->mOutputLetterbox, true);
...
void FGLRenderer::PresentSideBySide(int vrmode)
{
	if (vrmode == VR_SIDEBYSIDEFULL || vrmode == VR_SIDEBYSIDESQUISHED)
	{
		...
		int leftWidth = zx_screen->mOutputLetterbox.width / 2;
		int rightWidth = zx_screen->mOutputLetterbox.width - leftWidth;
		IntRect leftHalfScreen = zx_screen->mOutputLetterbox;
		leftHalfScreen.width = leftWidth;
		IntRect rightHalfScreen = zx_screen->mOutputLetterbox;
	...
	{
		mBuffers->BindOutputFB();
		zx_screen->mOutputLetterbox.top = zx_screen->mOutputLetterbox.height;

		ClearBorders();
		zx_screen->mOutputLetterbox.top = 0;  //reset so screenshots can be taken

		// Compute screen regions to use for left and right eye views
		int leftWidth = zx_screen->mOutputLetterbox.width / 2;
		int rightWidth = zx_screen->mOutputLetterbox.width - leftWidth;
		//cut letterbox height in half
		int height = zx_screen->mOutputLetterbox.height / 2;
		int top = height * .5;
		IntRect leftHalfScreen = zx_screen->mOutputLetterbox;
		leftHalfScreen.width = leftWidth;
...
void FGLRenderer::PresentTopBottom()
{
	...
	// Compute screen regions to use for left and right eye views
	int topHeight = zx_screen->mOutputLetterbox.height / 2;
	int bottomHeight = zx_screen->mOutputLetterbox.height - topHeight;
	IntRect topHalfScreen = zx_screen->mOutputLetterbox;
	topHalfScreen.height = topHeight;
	topHalfScreen.top = topHeight;
	IntRect bottomHalfScreen = zx_screen->mOutputLetterbox;
...
void FGLRenderer::prepareInterleavedPresent(FPresentShaderBase& shader)
{
	...
	const IntRect& box = zx_screen->mOutputLetterbox;
	...
	shader.Uniforms->Scale = {
		zx_screen->mScreenViewport.width / (float)mBuffers->GetWidth(),
		zx_screen->mScreenViewport.height / (float)mBuffers->GetHeight()
	};
...
void FGLRenderer::PresentColumnInterleaved()
{
	...
	// Todo:
	//auto clientoffset = zx_screen->GetClientOffset();
...
void FGLRenderer::PresentRowInterleaved()
{
	...
	mPresent3dRowShader->Uniforms->WindowPositionParity =
		(windowVOffset
			+ zx_screen->mOutputLetterbox.height + 1 // +1 because of origin at bottom
			) % 2;
