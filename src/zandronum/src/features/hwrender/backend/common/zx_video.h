...
#include "intrect.h"
#include "m_png.h"
// [rc4l] Dropped: UZDoom's 2D layer needs their FGameTexture/DAngle/ETexMode model, and we keep Zandronum's own 2D path.
#include "vectors.h"
...
struct HWDrawInfo;
class FMaterial;
// [rc4l] FGameTexture is a typedef for our FTexture; no forward declaration.
class FRenderState;
class BoneBuffer;
...
struct FColormap;
// [rc4l] Ours (textures.h) has no fixed underlying type, so drop the : uint32_t to avoid a conflicting redeclaration.
enum FTextureFormat;
...
{
public:
	ZXCanvas (int width, int height, bool bgra);
	~ZXCanvas ();
...
public:
	ZXFrameBuffer (int width=1, int height=1);
	virtual ~ZXFrameBuffer();
	...
	virtual ZXCanvas* GetCanvas() { return nullptr; }
...
extern ZXFrameBuffer *zx_screen;

#define SCREENWIDTH (zx_screen->GetWidth ())
#define SCREENHEIGHT (zx_screen->GetHeight ())
