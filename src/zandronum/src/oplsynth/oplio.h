...
#include <stdint.h>
#include <stdio.h>
#include "zstring.h"		// [rc4l] needed for FString in the retained DiskWriterIO.
...
// [rc4l] Retained from the old MUS library; the raw-OPL players and disk writer need it.
...
// [rc4l] Disk-writer OPL IO retained from the old MUS library so the OPL dumper still works.
struct DiskWriterIO : public OPLio
{
	DiskWriterIO(const char *filename);
	~DiskWriterIO();

	int Init(int core, uint32_t numchips, bool notused, bool notused2);
	void Reset();
	void WriteRegister(int which, uint32_t reg, uint8_t data);
