...
**Still on the sdl12-compat build:** `mac_build.sh` continues to build the 1.2 shim even though nothing
...
`sdl/i_main.cpp` from the SDL 1.2 API to SDL2 (already bundled; drop `sdl12-compat` from `mac_build.sh`).
...
- Engine: `cmake --build build-mac-arm`; then **`./mac_build.sh`** to refresh `build/ZandroX.app` — the
